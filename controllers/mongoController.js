const vCategorias = require('../modelos/categorias');
const vProductos = require('../modelos/producto');
const vProductosFotos = require('../modelos/ProductoFoto');
...
exports.categorias  = async (req, res) => {
    try {
        let datosCategorias = await vCategorias.find().lean().sort({ _id: -1 });
        return res.status(200).json(datosCategorias);
    } catch (error) {
...
/* {"nombre": "mi categoria desde rest"} */
